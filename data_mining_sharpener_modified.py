...
def reprojectSubsetLowResScene(highResScene, lowResScene, resampleAlg=gdal.GRA_Bilinear):
    ...
    # BR = pix2point(point2pix([extent[2], extent[1]], gt_LR, upperBound=False), gt_LR)
    # out = gdal.Warp("",
    ...
    data = lowResScene.GetRasterBand(1).ReadAsArray()

    out = saveImg(data,
            lowResScene.GetGeoTransform(),
            lowResScene.GetProjection(),
            "MEM",
            noDataValue=np.nan)
...
def resampleLowResToHighRes(lowResScene, highResScene, resampleAlg="cubic"):
    ...
    data = cv2.resize(data, dsize = (data.shape[0]*scale[0], data.shape[1]*scale[1]), fx = scale[0], fy = scale[1], interpolation=cv2.INTER_CUBIC)
...
class DecisionTreeSharpener(object):
    ...
    def residualAnalysis(self, disaggregatedFile, lowResFilename, lowResQualityFilename=None,
        ...
        # This two functions will be modified
        ...
        #  Residual LR and residual HR seems ok
        if self.disaggregatingTemperature:
            if doCorrection:
