...
Description
-----------
Training of the modelB using Sobel derivative filtering to compute gradients
and use them inside the perceptual loss. 
...
def train_step(model: torch.nn.Module, 
    ...
    gamma : float
        Perceptual loss hyperparameter.
    device : str
    ...
    for _, data in enumerate(pbar): 
        ...
        # FORWARD PASS
        lst, lst_up, ndvi = data[0].to(device), data[1].to(device), data[2].to(device) 
        
        optimizer.zero_grad()
        
        # For modelB_2
        lst_ndvi = torch.cat((lst_up, ndvi), dim = 1)
        
        lst_SR = model(lst_ndvi)
        
        # Downsampling the prediction # Need to unnormalize and renormalize
        mean = train_ds.stats['mean_lst']
        std = train_ds.stats['std_lst'] 
        lst_SR_unnorm = lst_SR * std + mean
        lst_SR_down = us.downscale_LST_SR_to_LR(lst_SR_unnorm)
        lst_SR_down = (lst_SR_down - mean)/std
        
        # Downsampling Loss (Error between LST low res and downscaled predict LST_SR)
        ds_loss = loss_fn(lst_SR_down, lst)
        
        filters_tensor = torch.zeros((len(filters),1,3,3))
        for i in range(len(filters_tensor)):
            filters_tensor[i,0,:,:] = torch.tensor(filters[i], dtype = torch.float)
        
        filters_tensor = filters_tensor.to(device)
        
        # Here we measure the gradients and try something new
        grads_lst = F.conv2d(lst_SR, filters_tensor, padding = 'same')
        grads_ndvi = F.conv2d(ndvi, filters_tensor, padding = 'same')

        percep_loss = loss_fn(grads_lst, gamma * grads_ndvi)
        
        ### Final loss
        loss = alpha * ds_loss + (1 - alpha) * percep_loss
        
        loss.backward()
        
        optimizer.step()
        
        ds_loss_train += ds_loss.item()
        percep_loss_train += percep_loss.item()
        loss_train += loss.item()
        psnr_train += us.psnr_skimage(lst_SR.detach().cpu().numpy(), lst_up.detach().cpu().numpy())
        ssim_train += us.ssim_skimage(lst_SR.detach().cpu().numpy(), lst_up.detach().cpu().numpy())
        
        pbar.set_description('Loss over the batch: ds_loss: {}, percep loss: {}'.format(np.format_float_scientific(ds_loss.item(), precision = 2), np.format_float_scientific(percep_loss.item(), precision = 2)))
    
    # Computing the mean of the metrics over the whole epoch.
    ds_loss_train /= len(train_dataloader)
    percep_loss_train /= len(train_dataloader)
    loss_train /= len(train_dataloader)
    psnr_train /= len(train_dataloader)
    ssim_train /= len(train_dataloader)

    return ds_loss_train, percep_loss_train, loss_train, psnr_train, ssim_train
...
def test_step(model: torch.nn.Module, 
    ...
    for _, data in enumerate(pbar): 
        ...
        ds_loss_test += ds_loss.item()
        percep_loss_test += percep_loss.item()
        loss_test += loss.item()
        psnr_test += us.psnr_skimage(lst_SR.detach().cpu().numpy(), lst_up.detach().cpu().numpy())
        ssim_test += us.ssim_skimage(lst_SR.detach().cpu().numpy(), lst_up.detach().cpu().numpy())
        
        pbar.set_description('Loss over the batch: ds_loss: {}, percep loss: {}'.format(np.format_float_scientific(ds_loss.item(), precision = 2), np.format_float_scientific(percep_loss.item(), precision = 2)))
    
    # Computing the mean of the metrics over the whole epoch.
    ds_loss_test /= len(test_dataloader)
    percep_loss_test /= len(test_dataloader)
    loss_test /= len(test_dataloader)
    psnr_test /= len(test_dataloader)
    ssim_test /= len(test_dataloader)

    return ds_loss_test, percep_loss_test, loss_test, psnr_test, ssim_test
...
def train(model: torch.nn.Module,
    ...
    metrics = {}
    metrics['train_loss'] = []
    metrics['train_dsloss'] = []
    metrics['train_perceploss'] = []
    metrics['train_psnr'] = []
    metrics['train_ssim'] = []
    metrics['val_dsloss'] = []
    metrics['val_perceploss'] = []
    metrics['val_loss'] = []
    metrics['val_psnr'] = []
    metrics['val_ssim'] = []
    ...
    for i in tqdm(range(1,n_Epochs+1,1), position = 0, desc = "Training", unit = " Epochs"):
            ...
            dl, pl, tl, tp, ts = train_step(model, 
                           filters,
                           train_dataloader,
                           optimizer,
                           loss_fn,
                           ...
                           gamma,
                           device)
            metrics['train_dsloss'].append(dl)
            metrics['train_perceploss'].append(pl)
            metrics['train_loss'].append(tl)
            metrics['train_psnr'].append(tp)
            metrics['train_ssim'].append(ts)
            
            dl, pl, tl, tp, ts = test_step(model, 
                           filters,
                           val_dataloader,
                           loss_fn,
                           alpha,
                           gamma,
                           device)
            metrics['val_dsloss'].append(dl)
            metrics['val_perceploss'].append(pl)
...
def plot_loss(metrics_dict, savepath = './results/modelB', modelname = 'modelB'):
    ...
    l1, = plt.plot(range(long), metrics_dict['train_dsloss'], color = 'blue')
    l2, = plt.plot(range(long), metrics_dict['val_dsloss'], color = 'orange')
    plt.legend(loc ='upper right', handles = [l1, l2], labels = ['Train dsLoss', 'Test dsLoss'])
    plt.title("loss = f(epoch)")
    plt.xlabel('epoch')
    plt.ylabel('loss_value')
    plt.savefig(os.path.join(savepath, modelname+'_dsloss.png'))
    
    plt.figure(5, figsize = (10,7))
    l1, = plt.plot(range(long), metrics_dict['train_perceploss'], color = 'blue')
    l2, = plt.plot(range(long), metrics_dict['val_perceploss'], color = 'orange')
    plt.legend(loc ='upper right', handles = [l1, l2], labels = ['Train perceploss', 'Test perceploss'])
    plt.title("loss = f(epoch)")
    plt.xlabel('epoch')
    plt.ylabel('loss_value')
    plt.savefig(os.path.join(savepath, modelname+'_perceploss.png'))
...
if __name__ == '__main__':
    ...
    train_ds = ModisDatasetB('data/ModisDatasetB.csv', transf = dataset_parameter['transf'], split = 'Train', time = dataset_parameter['time'])
    val_ds = ModisDatasetB('data/ModisDatasetB.csv', transf = dataset_parameter['transf'], split = 'Val', time = dataset_parameter['time'])
